---
layout: default
title: "Lab 7-a: Baseline Multi-Region Web App with Traffic Manager & Chaos Studio"
---

[Lab 7-b: Private Networking Variant →](lab-07b-webapp-traffic-manager-private-networking.md)

# Lab 7-a: Baseline Multi-Region Web App with Traffic Manager & Chaos Studio

This is the baseline **public-edge** variant of the Traffic Manager lab. It keeps the original App Service + Traffic Manager + Chaos Studio flow. If you want the spoke-based version that assumes the Lab 0 network foundation and moves supported dependency paths onto private networking, continue to **Lab 7-b**.
...
```bash
LAB_ROOT="$HOME/lab07a-webapp"
ZIP_PATH="$HOME/lab07a-webapp.zip"
...
```powershell
$LabRoot = Join-Path $HOME "lab07a-webapp"
$ZipPath = Join-Path $HOME "lab07a-webapp.zip"
...
CHAOS_JSON_PATH="$HOME/lab07a-chaos-experiment.json"

cat > "$CHAOS_JSON_PATH" <<JSONEOF
{
  "location": "${PRIMARY_REGION}",
  "identity": {
    "type": "SystemAssigned"
...
$SUBSCRIPTION_ID = az account show --query "id" -o tsv
$EXPERIMENT_NAME = "chaos-stop-primary-app"
$ChaosJsonPath = Join-Path $HOME "lab07a-chaos-experiment.json"
...
rm -rf "$HOME/lab07a-webapp" "$HOME/lab07a-webapp.zip" "$HOME/lab07a-chaos-experiment.json"
```
...
Remove-Item (Join-Path $HOME "lab07a-webapp") -Recurse -Force -ErrorAction SilentlyContinue
Remove-Item (Join-Path $HOME "lab07a-webapp.zip") -Force -ErrorAction SilentlyContinue
Remove-Item (Join-Path $HOME "lab07a-chaos-experiment.json") -Force -ErrorAction SilentlyContinue
```
...
[Continue to Lab 7-b: Private Networking Variant →](lab-07b-webapp-traffic-manager-private-networking.md)
