...
echo "============================================"
echo "  LAB 12: PostgreSQL Flexible Server Geo    "
echo "============================================"

RANDOM_SUFFIX=$(head /dev/urandom | tr -dc 'a-z0-9' | head -c 5)
PRIMARY_REGION="swedencentral"
SECONDARY_REGION="norwayeast"
RG_PRIMARY="rg-pgsql-dr-swc-${RANDOM_SUFFIX}"
RG_SECONDARY="rg-pgsql-dr-noe-${RANDOM_SUFFIX}"
...
DB_NAME="sampledb"
VARS_FILE="${LAB12_VARS_FILE:-$(mktemp /tmp/lab12_vars.XXXXXX.env)}"
...
echo ""
echo ">>> Step 2: Creating source PostgreSQL Flexible Server..."
...
REPLICA_ROLE=$(az postgres flexible-server show \
  --name "$REPLICA_SERVER" \
  --resource-group "$RG_SECONDARY" \
...
PROMOTED_ROLE=$(az postgres flexible-server show \
  --name "$REPLICA_SERVER" \
  --resource-group "$RG_SECONDARY" \
  --query "replicationRole" -o tsv 2>/dev/null || echo "unknown")
az postgres flexible-server show \
